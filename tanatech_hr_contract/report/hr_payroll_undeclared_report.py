...
class HrPayrollUndeclaredReport(models.Model):
    _name = "hr.payroll.undeclared.report"
    _description = "Undeclared Payroll Analysis Report"
    _auto = False
    _rec_name = 'date_from'
    _order = 'date_from desc'


    count = fields.Integer('# Undeclared Payslip', aggregator="sum", readonly=True)
    ...
    def _from(self, additional_rules):
        from_str = """
            FROM
                (SELECT * FROM hr_payslip WHERE state IN ('done', 'paid') AND is_from_undeclared_contract IS TRUE) p
    ...
    def _get_action(self):
        ...
        action = self.env['ir.actions.act_window']._for_xml_id('tanatech_hr_contract.payroll_undeclared_report_action')
