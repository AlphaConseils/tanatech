...
def button_validate(self):
	if self.show_validate and self.is_second_validator and self.state != 'second_validate':
		self.state = 'second_validate'
	
	elif not self.is_second_validator or not self.show_validate:
...
class Picking(models.Model):
	...
	is_second_validator = fields.Boolean(compute="compute_is_second_validator")
	
	def compute_is_second_validator(self):
		for picking in self:
			...
			valid = user_has_group_stock and not user_has_group_syst
			picking.is_second_validator = valid
	...
	def _compute_show_validate(self):
		for picking in self:
			...
			elif picking.state not in ('draft', 'waiting', 'confirmed', 'assigned', 'second_validate'):
				picking.show_validate = False
			else:
